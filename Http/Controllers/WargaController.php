...
class WargaController
{
    ...
    public function index()
    {
        ...
        // Logic Hak Akses RW vs RT menggunakan scope method di Model Warga
        if ($user->isRw()) {
    ...
    /**
     * 4. Memverifikasi / Approve Pengajuan dari RT (Khusus RW)
     */
    public function approve()
    {
        ...
        if ($wargaId) {
            $db = App::resolve(Database::class);
            $db->query("UPDATE " . Warga::$table . " SET status_verifikasi = 'verified' WHERE id = :id", [
                'id' => $wargaId
            ]);
        }
    ...
    /**
     * 5. Menolak Pengajuan dari RT (Khusus RW)
     */
    public function reject()
    {
        ...
        if ($wargaId) {
            $db = App::resolve(Database::class);
            $db->query("UPDATE " . Warga::$table . " SET status_verifikasi = 'rejected' WHERE id = :id", [
                'id' => $wargaId
            ]);
        }
